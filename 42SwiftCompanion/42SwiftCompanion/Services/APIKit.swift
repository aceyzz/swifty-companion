...
final class ProfileRepository {
    ...
    func heavyPieces(login: String) async throws -> (coalitions: [CoalitionRaw], coalitionUsers: [CoalitionUserRaw], projects: [ProjectRaw]) {
        ...
        async let projects: [ProjectRaw] = api.pagedRequest { page in
            Endpoint(path: "/v2/users/\(login)/projects_users", queryItems: [URLQueryItem(name: "page", value: "\(page)")])
        }
        return try await (coalitions: coalitions, coalitionUsers: coalitionUsers, projects: projects)
    }

    func fullProfile(login: String) async throws -> UserProfile {
        let user: UserInfoRaw = try await api.request(Endpoint(path: "/v2/users/\(login)"), as: UserInfoRaw.self)
        let pieces = try await heavyPieces(login: login)
        return assemble(user: user, pieces: pieces)
    }

    func assemble(user: UserInfoRaw, pieces: (coalitions: [CoalitionRaw], coalitionUsers: [CoalitionUserRaw], projects: [ProjectRaw])) -> UserProfile {
        let coalitionsMerged: [UserProfile.Coalition] = pieces.coalitions.map { c in
            let s = pieces.coalitionUsers.first { $0.coalition_id == c.id }
        ...
        let finished: [UserProfile.Project] = pieces.projects.filter {
        ...
        let active: [UserProfile.ActiveProject] = pieces.projects.filter {
        ...
        return UserProfile(raw: user, coalitions: coalitionsMerged, finishedProjects: finished, activeProjects: active, currentHost: nil)
    }
}
